...
that belongs to the `Logic` component yields a promising candidate in `com.homeboss.logic.Logic`.
...
A quick look at the `com.homeboss.logic.Logic` (an extract given below) confirms that this indeed might be what we’re
looking for.
